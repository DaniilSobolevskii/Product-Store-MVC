...
public class AdoDataContext : IDataContext
{
    ...
    private IDataContext _dataContextImplementation;
    ...
    public int InsertProduct(Product product)
    {
        ...
        return Products.Rows.Count+1;
    }

    public int UpdateProduct(int id, Product product)//product product
    {
        var row = Products?.Select($"id = {id}").FirstOrDefault();
    ...
    {
        return _dataContextImplementation.UpdateCustomer(id, customer);
    }
    ...
    public IList<Orders?> SelectOrders(int? id = null)
    {
        ...
        var rows = id.HasValue ? Orders.Select($"id = {id}") : Orders.Select();
    ...
    private Product dataRowToProduct(DataRow row) 
    {
        ...
        Product product = productType switch {
            ...
            ProductType.Movie => new Movie(),
            ProductType.Undefined => new Undefined(),
        ...
        product.Description = row.IsNull(2) ? "" :(string)row[2];
        product.Price = row.IsNull(3) ? 0 : (double)row[3];
        product.Count = (int)row[4];
